...
for i in range(0, 3):
    ...
    # Read the file
    root_main = pyhit.load('microwave-loosely-coupled-main-Plasma-75W.i')
    root_sub = pyhit.load('microwave-loosely-coupled-sub-EM-IC.i')

    # Rewrite Initial Condition File
    input_parameters = moosetree.find(root_main, func=lambda n: n.fullpath == '/Mesh/fmg')
    input_parameters["file"] = 'microwave-loosely-coupled-main-Plasma-IC_'+str(pressures_Torr[i])+'mTorr_exo_out.e'
    ...
    pyhit.write("microwave-loosely-coupled-main-Plasma-75W_"+str(pressures_Torr[i])+"mTorr.i", root_main)
    pyhit.write("microwave-loosely-coupled-sub-EM-IC_"+str(pressures_Torr[i])+"mTorr.i", root_sub)

    # This runs the modified input file
    input_files = ['microwave-loosely-coupled-main-Plasma-75W_'+str(pressures_Torr[i])+'mTorr.i']
    ...
    data = np.genfromtxt("microwave-loosely-coupled-main-Plasma-75W_"+str(pressures_Torr[i])+"mTorr_csv_out.csv",skip_header=1,delimiter=',')
